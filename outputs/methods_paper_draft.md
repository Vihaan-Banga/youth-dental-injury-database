...
- A small number of rows (≈15, flagged in `outputs/data_quality.md`) carry an **all-cause or head/neck injury rate** from a study whose injury definition merely *included* dental injuries (e.g. collegiate rugby all-injury incidence). These are not dental-specific rates and must not be compared as such; a rate-scope guard (`scripts/39`) surfaces them for reclassification or exclusion pending advisor review.
